  ...
  try {
    ...
    const user = await client.query(
      `insert into organization  (name, address, phone, icon)
         values ('${name}','${address}','${phone}','${icon}')  RETURNING id`,
    );
...
const getAllOrganizationDetail = async (req, res, next) => {
  try {
    // const id = req.params.id;
    const organizationDetail = await client.query(
      `   select o.id,o."name" ,o.phone ,o.address,o.created_at, (jsonb_agg(jsonb_build_object('coordinate1',p.coord1,'coordinate2',p.coord2,'coordinate3',p.coord3,'coordinate4',p.coord4,'coordinate5',p.coord5,'coordinate6',p.coord6)))as organization_coordinates from organization o join premises p on o.id=p.o_id where o.deleted_at is null group by o.id,o."name",o.phone ,o.address ,o.created_at     `,
    );
    if (organizationDetail.rowCount > 0) {
      res.status(200).json({
        success: true,
